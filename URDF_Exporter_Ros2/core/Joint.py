...
class Joint:
    def __init__(
        self, name, xyz, rpy, axis, parent, child, joint_type, upper_limit, lower_limit
    ):
        """
        Attributes
        ----------
...
def make_joint_dict(joint, links_dict, msg, all_joints):
    ...
    if joint_type == "revolute":
        ...
        if max_enabled and min_enabled:
            ...
            msg = (
                joint.name + "is not set its lower limit. Please set it and try again."
            )
            return None, None, msg
        elif not max_enabled and min_enabled:
            msg = (
                joint.name + "is not set its upper limit. Please set it and try again."
            )
            return None, None, msg
    ...
    try:
        ...
        def get_joint_geometry_matrix(geo_or_origin):
            ...
            if geo_or_origin.objectType == adsk.fusion.JointOrigin.classType():
                return (
                    geo_or_origin.getAsCoordinateSystem(),
                    f"{str(geo_or_origin.getAsCoordinateSystem())}",
                )  # coordinateSystem.copy(), None

            # Case 2: JointGeometry → get entity
            if (
                geo_or_origin.objectType == adsk.fusion.JointGeometry.classType()
                and geo_or_origin.isValid
            ):
                ent = (
                    geo_or_origin.entityTwo
                    if geo_or_origin.entityTwo is not None
                    else geo_or_origin.entityOne
                )
                ...
                # ---- Edges / Axes ----
                    ...
                    if ent.classType() == adsk.fusion.BRepEdge.classType():
                        ...
                        try:
                            ...
                            if (
                                ...
                                if hasattr(curve, "evaluator"):
                                    ...
                                    success, midpoint = evaluator.getPointAtParameter(
                                        mid_param
                                    )
                                    if success:
                                        # Get direction at midpoint for orientation
                                        success2, direction = evaluator.getTangent(
                                            mid_param
                                        )
                                        if success2 and direction.length > 1e-6:
                                            ...
                                            mat.setWithCoordinateSystem(
                                                midpoint, direction, y_axis, z_axis
                                            )
                                            return mat, None
                                        else:
                            ...
                            if hasattr(curve, "evaluator"):
                                ...
                                param_range = evaluator.parameterRange
                                mid_param = (param_range[0] + param_range[1]) / 2.0
                                success, point = evaluator.getPointAtParameter(
                                    mid_param
                                )
                                if success:
                        ...
                        if hasattr(geom, "origin") and hasattr(geom, "direction"):
                            ...
                            mat.setWithCoordinateSystem(
                                origin, direction, y_axis, z_axis
                            )
                            return mat, None
                        ...
                        if hasattr(geom, "startPoint") and hasattr(geom, "endPoint"):
                            ...
                            mat.setWithCoordinateSystem(
                                start_pt, direction, y_axis, z_axis
                            )
                            return mat, None
                ...
                # ---- Faces / Planes ----
                    ...
                    return (
                        None,
                        f"Unsupported JointGeometry entity type: {ent.classType()}",
                    )
        ...
        try:
            ...
            if joint.geometryOrOriginOne:
                ...
                if joint_dict["parent"] in links_dict:
                    ...
                    parent_joint = next(
                        (
                            ...
                            if re.sub(
                                "[ :()]",
                                "_",
                                j.occurrenceOne.name.split(" ", 1)[0].split(":", 1)[0],
                            )
                            == joint_dict["parent"]
                        ),
                        None,
                    ...
                    if joint_dict["parent"] not in links_dict:
                        if not parent_joint or (
                            parent_joint and not parent_joint.occurrenceTwo
                        ):
                ...
                if geometry_type == "construction_point":
                    ...
                    transform_matrix = utils.relative_transform(
                        utils.list_to_matrix(transform2.asArray()),
                        utils.list_to_matrix(transform1.asArray()),
                    )
            ...
            if transform is None or (
                ...
                and geometry_type not in ["construction_point", ""]
            ):
            ...
            roll, pitch, yaw = utils.get_rpy_from_matrix(
                utils.matrix_to_list(transform_matrix)
            )
        except Exception as e:
            msg = (
                f"1. Error creating child transform for {joint_dict['child']}: {str(e)}"
            )
            return None, None, msg
        try:
            ...
            if joint.geometryOrOriginOne:
                if geometry_type == "construction_point":
                    ...
                    joint.geometryOrOriginOne.objectType
                    == adsk.fusion.JointGeometry.classType()
                ):
                    ent = (
                        joint.geometryOrOriginOne.entityTwo
                        if joint.geometryOrOriginOne.entityTwo is not None
                        else joint.geometryOrOriginOne.entityOne
                    )
                    if ent:
